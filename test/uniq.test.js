...
test('クリックした階層内ではピン留めタブをトップレベルとして扱い優先して残す', async () => {
  resetTabs([
    ...
    { id: 3, windowId: 1, index: 2, active: true, pinned: false, url: 'https://example.com/', title: 'Example' },
  ])

  await run(1, 'url', false, 'currentHierarchy', {
    groupId: -1,
    pinned: false,
  })
...
test('トップレベル指定では右クリック元グループとトップレベルを対象にしピン留めを残す', async () => {
  resetTabs([
  ...
  await run(1, 'url', false, 'topLevelHierarchy', {
    groupId: 10,
    pinned: false,
  })

  assert.deepEqual(state.removed, [2, 3, 1])
...
test('全ての階層ごとではピン留めと通常トップレベルを同じ階層で判定する', async () => {
  ...
  assert.deepEqual(state.removed, [2, 4])
...
test('urlWithoutHash では URL ハッシュ違いを重複として扱う', async () => {
  ...
  await run(1, 'urlWithoutHash', false)
...
test('全てのタブではタブグループをまたいで重複タブを閉じる', async () => {
  ...
  await run(1, 'url', false, 'ignoreBoundaries')

  assert.deepEqual(state.removed, [2])
})

test('全てのタブではアクティブな通常タブよりピン留めタブを残す', async () => {
  resetTabs([
    { id: 1, windowId: 1, index: 0, active: false, pinned: true, url: 'https://example.com/', title: 'Example' },
    { id: 2, windowId: 1, index: 1, active: true, pinned: false, url: 'https://example.com/', title: 'Example' },
  ])

  await run(1, 'url', false, 'allTabs')
...
test('全てのタブでピン留めタブがない場合はアクティブな重複タブを残す', async () => {
  resetTabs([
    { id: 1, windowId: 1, index: 0, active: false, pinned: false, groupId: 1, url: 'https://example.com/', title: 'Example' },
    { id: 2, windowId: 1, index: 1, active: true, pinned: false, groupId: 2, url: 'https://example.com/', title: 'Example' },
  ])
