...
async function hideMenu () {
  for (const listener of events.menusHidden.listeners) {
    await listener()
  }
}

async function waitForMenuCountUpdate () {
  await new Promise((resolve) => globalThis.setTimeout(resolve, 0))
  await new Promise((resolve) => globalThis.setTimeout(resolve, 0))
}
...
test('ロード時にメニューを復元する', async () => {
  assert.ok(state.menuItems.has('uniq'))
  assert.notEqual(state.menuItems.get('uniq').visible, false)
  assert.equal(state.menuItems.get('uniq:action').visible, false)
...
test('表示処理は進行中のメニュー復元完了を待つ', async () => {
  ...
  resetState({
    menuItems: { title: ['currentHierarchy'] },
    tabs: [
  ...
  assert.equal(state.menuItems.get('uniq').visible, false)
  assert.equal(state.menuItems.get('uniq:action').visible, true)
  assert.equal(state.menuItems.get('uniq:action').title,
    'uniq: title: topLevelScope')
...
test('表示後に非同期でルート項目へ重複数を追加する', async () => {
  resetState({
  ...
  assert.equal(state.menuItems.get('uniq:action').title,
    'uniq: url: topLevelScope')
  assert.equal(state.refreshCount, 1)

  await waitForMenuCountUpdate()

  assert.equal(state.menuItems.get('uniq:action').title,
    'uniq: url: topLevelScope (1)')
  assert.equal(state.refreshCount, 2)
})

test('表示後に非同期でサブメニュー項目へ重複数を追加する', async () => {
  resetState({
    menuItems: { url: ['currentHierarchy', 'eachHierarchy', 'allTabs'] },
    tabs: [
      {
...
test('グループ内タブのトップレベル項目はグループ内重複を数えない', async () => {
  resetState({
    menuItems: { url: ['currentHierarchy', 'eachHierarchy', 'allTabs'] },
    tabs: [
      {
...
test('メニューが閉じられた後は重複数を反映しない', async () => {
  resetState({
    menuItems: { url: ['currentHierarchy'] },
    tabs: [
      {
...
test('ピン留めタブではクリックした階層内をトップレベルとして表示する', async () => {
  resetState({
    menuItems: { url: ['currentHierarchy', 'allTabs'] },
    tabs: [
      { id: 1, windowId: 1, active: true, pinned: true },
...
test('全ての階層ごとではピン留めと通常トップレベルを同じ階層で判定する', async () => {
  resetState({
  ...
  assert.deepEqual(state.removed, [2, 4, 1, 7])
})
